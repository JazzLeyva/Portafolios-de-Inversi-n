...
if (EXPERIMENTO == 1) { # FALSE, VOO Y MCN
  ...
  dt3 <-data.table(read.csv(link3, header=TRUE))
    ...
    if(EXPERIMENTO == 2) { # NEE, NESN.S Y NVDA
        dt1 <-data.table(read.csv(link4, header=TRUE))
        dt2 <-data.table(read.csv(link5, header=TRUE))
        dt3 <-data.table(read.csv(link6, header=TRUE))
    } else {# NIO, PALAF Y QQQ
        dt1 <-data.table(read.csv(link7, header=TRUE))
        dt2 <-data.table(read.csv(link8, header=TRUE))
        dt3 <-data.table(read.csv(link9, header=TRUE))
    }
} 
...
plot_three <- ggplot(dt, aes(x = date, y = idx_price, color = ticker)) +
  geom_line() +
  # Miscellaneous Formatting
  theme_bw() + ggtitle("Precio desarrollado") +
  xlab("Fecha") + ylab("Precio\n(Indexed 2000 = 1)") +
  scale_color_discrete(name = "Compania")
ggsave(filename =  paste(c("resultados/Experimento",EXPERIMENTO,"/grafica_de_acciones.png"),collapse=""), plot_three, scale = 1.5, dpi=600)
...
ggsave(filename =  paste(c("resultados/Experimento",EXPERIMENTO,"/grafica_de_correlaciones.png"),collapse=""), plot_three, scale = 1.5, dpi=600)